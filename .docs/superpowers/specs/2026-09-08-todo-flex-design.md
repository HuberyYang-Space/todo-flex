...
> **2026-09-24 修订（取整误报，真实 Chrome 复现）**
>
> 实现里的尺寸其实一直读的是 `offsetWidth` / `offsetHeight`，与上面写的不符。它们取整，误报出在两处：
> 盒子实际 17.5 读成 18，与理论 17.4946 差 0.505，越过诊断的 0.5 容差；
> 叠加层每行「剩余 · 理论」把各盒子的取整误差累加起来，偏小数排版的 150 个随机状态里有 42 行被错标成不一致。
>
> - **尺寸改读计算样式的小数值**（`getComputedStyle`，同样不含 transform）。ResizeObserver 只负责通知「该重采了」。
>   不读它的 `borderBoxSize`：尺寸只在回调里给，状态变化触发的那条采样路径拿不到，得另存一份缓存，还要论证读到旧值的时序窗口
> - **位置仍是整数的 `offsetLeft` / `offsetTop`**：没有不含 transform 的小数读法。两个取整的位置最多差出 1px，
>   叠加层画色块的阈值因此从 0.5 放宽到 1——原先连 211px 三等分这种布局都会在紧贴的盒子之间画出 1px 的假色块
>
> 修后两批随机状态（常规 120 个、偏小数 150 个）与「静止时的真实小数几何」逐项对照：没归因的诊断 0 条，
> 行级不一致与标准答案条数相同，色块 0 处不同。标准答案里最窄的真实色块 1.5px，放宽阈值没有误删。
...
> **2026-09-24 修订（兜底解释误导，真实 Chrome 复现）**
>
> 本站没有任何 max 类属性，`max-size-clamp` 接住的其实全是**连带**：同一行的 A 被 `min-width: auto` 兜住后多占了空间，
> 其余盒子分到的跟着变小（三个 basis 300 挤进 500：B、C 从 158.7 缩到 113；伸展时 152 → 128），却被说成「尺寸被某个上下限截断了」。
> 规则改为三条：
>
> - `min-width-auto` 带上该行分配前的剩余空间，展示层据此分伸缩两种说法——伸展时盒子也会被内容撑住，「收缩到此为止」说错了方向
> - 新增 `min-width-auto-sibling`：自己没被撑住、实际比理论小，且同行有盒子被 `min-width: auto` 兜住并比理论多占了空间 → 点名源头（`causedBy`）
> - 兜底改名 `unexplained`，如实说「没能归到已知规则」；比理论大的偏差不硬套连带
>
> 随机 120 个状态在真实 Chrome 里扫过：连带解释出现 71 次（原先全被误判），`unexplained` 只剩 1 次——
> 是观测层用 `offsetWidth` 取整造成的（实际 17.5 读成 18，理论 17.4946，差 0.505 越过容差），已修，见 [6.2 节的修订](#62-观测层实际值)。
...
控制面板由 `data/flexProperties.ts` 驱动生成。

> ⚠️ **「更多值」折叠区已于 2026-09-20 移除，本节下面两张表的「折叠区补充」一列现在一律直出。**
> 当初担心穷举近义值会把面板撑爆，实际用下来不成立：操作区改成内部滚动之后高度不再稀缺，
> 而折叠反倒让 `start` / `left` / `self-start` 这些最该被看见的近义值藏了起来——
> 「把规范的全集摊开给人看」本来就是这个站的立身之本。
> 另外盒子属性那侧的折叠开关从来没渲染过，那半边功能始终是死的。
> 不要据着下面的表把折叠区加回来。
