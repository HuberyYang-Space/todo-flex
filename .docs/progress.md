...
| M6 打磨 | 🚧 URL 分享已提前落地；暗亮主题打磨未动；i18n 已移出 | 见下面「下一件事」 |
...
**M6 的剩余部分：暗亮主题打磨。**（URL 分享已提前落地，i18n 已移出——
i18n 为什么移出见 [pitfalls.md](./pitfalls.md#i18n-预埋字段为什么被删净)。）
...
> 已还清：状态 → CSS 的映射逻辑曾经有三份（`TrapStage` / `DemoStage` / `cssEmit`），
> 已统一到 `src/core/styleMap.ts`（commit `5adae3c`）。
