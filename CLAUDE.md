...
**M4 的两项收尾已写完，但一次浏览器都没跑过**（工作区当时未提交，5 个文件：
`core/overlay.ts`、`OverlayLayer.vue` 及三份 spec）。证据只到 `pnpm test` 188 passed + lint + build 全绿，
**功能效果完全没有运行验证**——当时 Chrome 扩展没连上（`list_connected_browsers` 返回 `[]`），
dev server 又被系统按内存不足杀了两次。下次务必在真实窗口里补验这四条：

1. 流动方向读不读得出来：默认 `flex-start` 时行尾色块的斜纹应朝**左**（流回前面的盒子）；
   改成 `flex-end` 后行首色块应反过来朝**右**。
2. **column 布局下方向还成不成立**——这是最大的风险点，见下条关于理发店转灯错觉的说明。
3. 2.4s 的周期会不会抢戏（斜纹是背景元素，动得太显眼就喧宾夺主）。
4. 系统打开「减少动态效果」后是否真的完全静止。

实现里有个反直觉的决策：45° 斜纹只能表达**垂直于自身**的运动分量（理发店转灯错觉），
所以「向右流」和「向下流」在 pattern 内部是同一个平移，**只做了 2 份 pattern 而不是 4 份**。
方向感靠色块自身的长宽比给出：row 下色块宽扁读作横向流，column 下高瘦读作纵向流。
**如果第 2 条核对下来 column 方向读不出来，就是这个假设不成立**，得拆成四份 pattern 重做。

等距实体块还有三项待用户在真实窗口里核对：
