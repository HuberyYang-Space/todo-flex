...
- **伸出去的顶面与右侧面只由 `.stage-wrapper` 的 `--overhang` 兜住**（`motion.stageOverhang`）——
  余量小于「顶面厚度 × `blockDepthHover` + `liftHeight` + 放大量」就会被外层 `.overflow-auto` 裁掉，
  `DemoStage.spec.ts` 的守卫钉着这个阈值，改 `blockDepth` / `liftHeight` 会让它变红。
  **不要给演示区滚动容器加内边距去"补"余量**：那是第二份留白，只把演示区往右下推、
  白缩可视范围，防裁切能力一点不增加（曾经这么干过，左侧留白 56px），`ThePlayground.spec.ts` 有守卫拦着。
  → [详见](./.docs/pitfalls.md#伸出去的面靠-overhang-兜住不是靠滚动容器的内边距)
